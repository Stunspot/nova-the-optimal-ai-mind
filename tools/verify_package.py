...
def verify(package: Path) -> dict[str, object]:
    ...
    if codex_manifest.get("name") != "nova-the-optimal-ai" or codex_manifest.get("version") != "3.1.2":
        findings.append("Codex plugin identity/version mismatch")
    if claude_manifest.get("name") != "nova-the-optimal-ai" or claude_manifest.get("version") != "3.1.2":
    ...
    for label, manifest in (("AnswerLayer", answer_manifest), ("Current Intelligence", current_manifest)):
        if manifest.get("rights_status") != "public-inclusion-authorized-for-nova-free-3.1.2":
            findings.append(f"{label} rights metadata is not reconciled for Nova Free")
        if "Nova Free 3.1.2 public split license" not in str(manifest.get("license", "")):
    ...
    for skill_id in sorted(EXPECTED_ROOTS):
        zip_path = zips_root / f"{skill_id}-3.1.2.zip"
        ...
        if record.get("claude_zip") != f"claude/zips/{skill_id}-3.1.2.zip":
