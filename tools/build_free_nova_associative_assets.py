...
EVIDENCE_PATH = ROOT / "verification" / "free-nova-associative-profile.md"
EXPECTED_EMBEDDING_PROFILE_ID = "embedding:ollama-qwen3-embedding-0.6b-nova-free-unqualified-r2"
EXPECTED_SNAPSHOT_ID = "snapshot:nova-free:qwen3-0.6b:unqualified-r2"
EXPECTED_ACTIVATION_ID = "activation:nova-free:qwen3-0.6b:unqualified-r2"
...
EXTRA_SPEC_PATH = ROOT / "design" / "free-nova-extra-capabilities.json"
...
def _require_rebind_identity(manifest: dict[str, Any]) -> None:
    ...
    expected_counts = {"expected_card_count": 41, "expected_relation_count": 33, "expected_vector_count": 246}
    ...
    if len(manifest.get("cards", [])) != 41 or len(manifest.get("relations", [])) != 33 or len(manifest.get("vectors", [])) != 246:
...
def main() -> int:
    ...
    cards_source = {"format":"mind-authored-capability-cards/v1","product":"Nova + MIND Free","product_version":"2.0.0",
      "purpose":"Public-safe semantic reminder surfaces for the complete Free Nova capability estate.",
    ...
    for h,n,c,p,b,*_ in SPECS:
        capabilities.append({"capability_id":f"capability:{h}","handle":h,"name":n,"product_id":"product:nova-free",
    ...
    bootstrap={"format":"mind-core-bootstrap/v1",
      "sources":[{"source_id":source_id,"locator":"bundle/reminder/associative-capability-cards.json","digest":source_digest,"custody_state":"canonical-constructed","authority_ref":"Nova + MIND Free 2.0.0 reviewed associative card source","observed_at":CREATED_AT}],
    ...
    lexical={"lexical_profile_id":"lexical:nfkc-contiguous-v1","name":"Nova Free exhaustive lexical cue profile",
    ...
    profile={"embedding_profile_id":"embedding:ollama-qwen3-embedding-0.6b-nova-free-unqualified-r2","name":"Ollama qwen3-embedding 0.6b Free Nova unqualified neighborhood",
      "provider_id":provider_id,"model_id":MODEL,"dimensions":dims,"metric":"cosine_distance","radius":0.33,
      "comparison_tolerance":1e-6,"vector_encoding":"float32_le","qualification_state":"unqualified",
      "qualification_evidence_ref":"verification/free-nova-associative-profile.md",
    ...
    for i,(a,b,k) in enumerate(RELATIONS,1):
        x={"capability_relation_id":f"relation:nova-free:{i:02d}:r1","from_capability_card_id":f"card:{a}:r1",
    ...
    snapshot={"associative_index_snapshot_id":"snapshot:nova-free:qwen3-0.6b:unqualified-r2","embedding_profile_id":profile["embedding_profile_id"],
      ...
      "builder_identity":"Nova Free deterministic associative asset builder","evidence_boundary":"Reviewed views plus local model vectors; this profile is unqualified until separate behavioral and fresh-host evidence passes.",
