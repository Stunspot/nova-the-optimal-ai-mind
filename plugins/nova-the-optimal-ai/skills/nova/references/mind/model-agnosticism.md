...
Engine 1.0.1 has no contract for adjusting dependent observations. A nonempty `dependence_refs` disclosure therefore stops validation and inference with `UNMODELED_OBSERVATION_DEPENDENCE` before filtering, likelihood, fit, decoding, smoothing, weighting, or reframing is computed. Do not “repair” the refusal by deleting the disclosure. Re-encode the evidence or use a model whose observation and calibration contracts explicitly represent the dependence.

Engine 1.0.0 did not enforce that boundary: it accepted nonempty `dependence_refs` and still computed likelihood, fit, and sometimes relative weights. Treat those probabilistic fields as supported only after recovering the exact input sequence, matching its canonical digest to `inputs.sequence_digest`, verifying the receipt identifier and engine artifact identity, and confirming that every `dependence_refs` array is empty. If the input is missing, mismatched, or dependent, mark the probabilistic result `unsupported/reframe`. Preserve the historical receipt bytes and identifiers; never delete a disclosure or rewrite and re-hash the receipt to make it look current.
...
A declared rival-model prior must be strictly positive. Exact zero would make an admitted rival unrecoverable under Bayesian updating, so it is rejected rather than masquerading as extreme skepticism; use null when between-model weighting is intentionally unsupported. Inside a model's initial, transition, or emission distributions, exact zero is a deliberate support exclusion justified by the existing `parameter_basis`: the supplied model does not represent that possibility. It is never a claim that the event cannot occur in the world. Use a small positive probability for an event the model considers extraordinarily unlikely, however contrived.

When an observation leaves a supplied model's support, `zero_likelihood_at_sequence_index` names the first such evaluated row. That trigger row has model-relative predictive probability `0.0` and no posterior. Later aligned rows contain null probability and posterior fields because inference stopped; they are not additional zero-probability judgments. Historical engine-1.0.0 receipts may show padded `0.0` values after the named index. Interpret those later legacy rows as not evaluated without rewriting or re-hashing the historical receipt. If every admitted model leaves support, Trellis requires REFRAME rather than treating the surprise as vindication.
