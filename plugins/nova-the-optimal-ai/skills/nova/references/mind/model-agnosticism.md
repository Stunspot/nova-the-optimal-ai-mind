...
A declared rival-model prior must be strictly positive. Exact zero would make an admitted rival unrecoverable under Bayesian updating, so it is rejected rather than masquerading as extreme skepticism; use null when between-model weighting is intentionally unsupported. Exact zero remains valid inside a model's initial, transition, or emission distributions only for a structural impossibility explicitly justified by the existing parameter_basis. Structural means impossible only within that explicitly bounded supplied model, never metaphysically or universe-wide impossible. Observing a structurally forbidden event gives that model zero likelihood; if every admitted model does so, Trellis requires REFRAME rather than treating the surprise as vindication. Use a small positive probability, not zero, for an event judged merely unlikely.
