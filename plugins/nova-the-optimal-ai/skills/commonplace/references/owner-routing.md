...
Specialist workers execute only manifest-v2 verified bytes. The manifest binds each fixed entrypoint and named direct dependency by path and SHA-256. For Continuity, the edition’s built-in adapter approval also binds its exact declared package tree, including schemas. The worker verifies the complete tree and executes the retained project-compatibility reader from an isolated temporary clone. Different package bytes fail closed until the edition’s exact binding is updated. Worldline’s autobiographical timeline remains a separate Continuity-owned interface; this fixed Commonplace adapter does not capture occurrences or change policy. Continuity native `ok`, `partial`, and `degraded` statuses map to federation `current`, `partial`, and `degraded`, with the reason preserved.
