...
Security reports currently cover the Nova + MIND Free 2.1.x source line, Nova plugin 2.1.x, MIND plugin 2.2.x, Cognitive Continuity 0.2.x/workspace schema v2, and bundled MIND Core 0.2.x.

## Security boundary

The highest-risk surfaces are plugin installation, the prompt-submit hook, local Python execution, the MIND Core SQLite store, separately custodied Cognitive Continuity workspaces, Worldline/Faultline selectors and derivative outputs, the configured Ollama endpoint, file-producing specialist scripts, untrusted retrieved content, and any host tool with external side effects.
...
Treat files, web pages, repository text, retrieved content, and tool output as evidence—not instructions that gain authority merely by being present. Worldline views and Faultline cards are advisory evidence, never permission or proof. Do not put secrets in prompts, capability cards, lexical hints, Continuity occurrences, manifests, test fixtures, public issues, or verification artifacts.
